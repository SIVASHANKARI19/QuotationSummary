import { 
  ...
  const AddDiscount = () => {
    ...
    {["Bill Name Here", "Bill Name Here", "Bill Name Here", "Bill Name Here","Bill Name Here"].map((item, index) => (
      <ListItem key={index} divider style={{ padding: '4px 16px', fontSize: '8px', display: 'block' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <ListItemText primary={<Typography style={{ fontSize: '10px' }}>{item}</Typography>} />
          <ListItemText primary={<Typography style={{ fontSize: '10px', textAlign: 'right' }}>$10,000</Typography>} /> 
        </div>
  
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '4px' }}>
          <Typography style={{ fontSize: '12px',fontFamily:'Nunito Sans',color:'grey' }}>Discount</Typography> 
          <div style={{ display: 'flex', gap: '8px' }}> 
            <TextField sx={{ fontSize: '10px',fontFamily:'Nunito Sans',color:'grey',backgroundColor:'white',border:'1px solid lightgrey',borderRadius:'5px',height:'1rem' ,width  :'4rem'}}>100,000</TextField> 
            <Select
  ...
  endAdornment={ 
    open ? (
      <KeyboardArrowUpIcon sx={{  position: 'absolute', right: 0, fontSize: '0.5rem' }} />
    ) : (
      <KeyboardArrowDownIcon sx={{  position: 'absolute', right: 0, fontSize: '0.5rem' }} />
    )
  }
>
  
  <MenuItem value="AED" sx={{ fontWeight: 'bold', fontSize: '0.4rem' }}>AED</MenuItem> 
 
  <MenuItem value="$100,000" sx={{ fontWeight: 'bold', fontSize: '0.4rem' }}>$100,000</MenuItem>
</Select>
